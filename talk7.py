...
def speak_bouyomi(text='ゆっくり簡易化ツールが起動しました。ベータ7.2', volume=-1, speed=-1, tone=-1):
    res = requests.get(
...
def ENTER(event):
    ...
    if(yomi == "exit"):
        ...
        speak_bouyomi("速度(200)" + times, int(vol.get()))
        yomi = times
        ...
        if(counting == 2.0):
            ...
            count()
        speak_bouyomi(yomi, int(vol.get()))
        if(yomi != ''):
            text_widget.configure(state='normal')
            text_widget.insert(
                counting, yomi + '\n')
